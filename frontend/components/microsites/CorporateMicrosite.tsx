...
export default function CorporateMicrosite({ event, onRegister }: CorporateMicrositeProps) {
  ...
  const dateValue = event.date || event.startDate || new Date();
  const eventDate = typeof dateValue === 'string' ? new Date(dateValue) : dateValue;
  const eventTitle = event.title || event.eventName || 'Corporate Event';
  const eventDescription = event.description || 'Join us for this exclusive corporate event';
  const eventLocation = event.location || event.venue || 'TBA';
  const eventCategory = event.category || 'Corporate Event';
  ...
  // Get speakers and agenda from event data
  const speakers = event.speakers || [];
  const agenda = event.agenda || event.schedule || [];
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-transparent">
                  {event.organizer?.company || event.organizer?.name || 'Event Organizer'}
                </h1>
                ...
                <Card key={index} className="overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
                  <div className="relative">
                    ...
                    <div className="absolute bottom-4 left-4">
                      ...
                      <p className="text-blue-200 text-sm">{speaker.title || speaker.role}</p>
                    </div>
                  </div>
                  <CardContent className="p-6">
                    <p className="text-gray-600">{speaker.bio || speaker.description || 'Expert speaker'}</p>
                  </CardContent>
                </Card>
              ))}
            </div>
      ...
      <section className="py-16 bg-gradient-to-r from-blue-50 to-indigo-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12">
            {/* Company Information */}
            <div>
              <h2 className="text-3xl font-bold text-gray-900 mb-8">About {event.organizer?.company || event.organizer?.name || 'the Organizer'}</h2>
      ...
      <section id="register-section" className="py-20 bg-white">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-600">Secure your spot at this exclusive event</p>
          </div>
          <Card className="shadow-2xl">
            <CardContent className="p-8">
              <p className="text-center text-gray-600 mb-6">
                Registration for this event is managed through our event platform.
              </p>
              <div className="text-center">
                <Button
                  size="lg"
                  className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white shadow-lg"
                  onClick={() => window.location.href = `/event/${event.slug}/register`}
                >
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-8">
            ...
            <div className="text-blue-200">
              <span className="font-semibold">{maxParticipants - registeredCount}</span> seats remaining
            </div>
          </div>
